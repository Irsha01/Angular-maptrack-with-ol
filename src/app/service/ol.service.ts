...
import { coordinates, AuthMapService } from '../service/auth-map.service';
...
export class OpenlayerMapService implements AuthMapService {
  ...
  addMarker(map: any, latLng: coordinates, id: string, image: string) {
    const feature = new olFeature({
      geometry: new Point(
        fromLonLat([latLng.lng, latLng.lat, 'EPSG:4326', 'EPSG:3857'])
      ),
      name: 'contentString',
      title: 'fddf',
    });
  ...
  setCenter(map: any, latLng: coordinates) {
    map.getView().setCenter(fromLonLat([latLng.lng, latLng.lat]));
  }
  ...
  moveMarker(
    ...
    if (map) {
      let feature = map.getLayers().item(1).getSource().getFeatureById(id);
      if (feature != null) {
  ...
  createLine(map: any, coordinates: any) {
    ...
    for (var i = 0; i < coordinates.length; i++) {
      points.push(fromLonLat([coordinates[i]['lng'], coordinates[i]['lat']]));
    ...
    map.getLayers().item(2).getSource().addFeature(featurething);
  ...
  fitBounds(map: any, item: any) {
    var extent = map.getLayers().item(item).getSource().getExtent();
  ...
  fitBoundsForCluster(map: any) {
    var extent = map.getLayers().item(1).getSource().getSource().getExtent();
    if (extent[0] != 'Infinity' && extent[0] != '-Infinity')
      map.getView().fit(extent, {
        duration: 1000,
        maxZoom: 16,
  ...
  removeMarkersById(map: any, id: string) {
    var deleteId = map.getLayers().item(1).getSource().getFeatureById(id);
    if (deleteId != null) {
      map.getLayers().item(1).getSource().removeFeature(deleteId);
    }
  }
